  ...
  switch (action.type) {
          ...
          [action.text]: {
            $merge: {
              ...
              translations: action.translations.filter(translation => {
                return (
                  translation.text.toLowerCase() !== action.text.toLowerCase()
                );
              }),
            },
          },
    ...
    case RIGHT_ANSWER:
      const correctAnswers =
        state.textInfoById[action.question].correctAnswers || 0;
      return update(state, {
        textInfoById: {
          [action.question]: { correctAnswers: { $set: correctAnswers + 1 } },
        },
        currentQuestion: {
          $merge: {
            isCorrect: true,
    ...
    case WRONG_ANSWER:
      const incorrectAnswers =
        state.textInfoById[action.question].incorrectAnswers || 0;
      return update(state, {
        textInfoById: {
          [action.question]: {
            incorrectAnswers: { $set: incorrectAnswers + 1 },
          },
        },
        currentQuestion: {
          $merge: {
    ...
    case SET_CURRENT_QUESTION:
      return update(state, {
        currentQuestion: { $set: action.currentQuestion },
      });
    case TOGGLE_CARD_LIST:
      return update(state, {
        cardListVisible: { $set: !state.cardListVisible },
      });
    default:
      return state;
  }
